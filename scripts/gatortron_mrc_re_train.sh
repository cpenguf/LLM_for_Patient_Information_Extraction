...
# GatorTron relation extraction with MRC framework using ClinicalTransformerMRC
# Source reference: https://github.com/uf-hobi-informatics-lab/ClinicalTransformerMRC (ADE relation script)
...
OUTPUT_DIR=${OUTPUT_DIR:-outputs/gatortron_mrc_re}
...
python "${CTMRC_ROOT}/src/train/mrc_ner_trainer.py" \
  ...
  --accumulate_grad_batches ${GRAD_ACC:-4} \
  ...
  --optimizer ${OPTIMIZER:-torch.adam} \
